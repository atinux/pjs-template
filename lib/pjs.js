...
function handleCache(options, template) {
  ...
  if (options.cache) {
    ...
    if (!path) {
      throw new Error('Internal PJS error: no file name or template provided');
    }
    template = fs.readFileSync(path).toString().replace(BOM, '');
  }
 ...
 * @param {String} str      PJS source
 * @param {String} filename file name of the PJS file
 ...
 * options object.
 *
 * This is used for compatibility with earlier versions of PJS and Express.js.
 *
 * @memberof module:pjs-internal
 ...
 *
 * @param {String}  template PJS template
 *
 ...
 * call this function with `data` being an empty object or `null`.
 *
 * @param {String}             path     path to the PJS file
 * @param {Object}            [data={}] template data
 * @param {Options}           [opts={}] compilation and rendering options
