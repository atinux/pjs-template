...
var fs = require('fs')
  ...
  , _REGEX_STRING = '(<%%|<%=|<%-|<%_|<%#|<%|%>|-%>|_%>)'
  , _OPTS = ['cache', 'filename', 'watchFiles', 'delimiter', 'debug', 'compileDebug', 'escape']
  , _TRAILING_SEMCOL = /;\s*$/
  , _BOM = /^\uFEFF/;
...
function Template(text, opts) {
  ...
  options.localsName = _DEFAULT_LOCALS_NAME;
...
, compile: function () {
    ...
    var returnedFn = function (data, callback) {
      return fn.apply(null, [data || {}, callback, escape, trycatch, rethrow]);
    };
    return returnedFn;
  }
        ...
        if ((include = line.match(/^\s*include\s+(\S+)/))) {
          ...
          if (opening && (opening == '<' + d || opening == '<' + d + '-' || opening == '<' + d + '_')) {
            ...
            try {
              ...
              throw new Error("Recursive templates include '"+include[1]+"'" + (self.opts.filename ? " in " + self.opts.filename : ""));
              return;
            }
            // includeSrc = '    ; (function(){' + '\n' + includeSrc + ' ; })()' + '\n';
